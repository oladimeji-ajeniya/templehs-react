...
/**
 * The TemplehsRouteItemType type is a custom type that extends the RouteObject type from react-router-dom.
 * It adds an optional auth property and an optional settings property.
 */
export type TemplehsRouteItemType = RouteObject & {
	auth?: string[] | [];
	settings?: unknown;
};

/**
 * The TemplehsRoutesType type is a custom type that is an array of TemplehsRouteItemType objects.
 */
export type TemplehsRoutesType = TemplehsRouteItemType[];

/**
 * The TemplehsRouteConfigType type is a custom type that defines the configuration for a set of routes.
 * It includes an optional routes property, an optional settings property, and an optional auth property.
 */
export type TemplehsRouteConfigType = {
	routes: TemplehsRoutesType;
	settings?: unknown;
	auth?: string[] | [];
};

/**
 * The TemplehsRouteConfigsType type is a custom type that is an array of TemplehsRouteConfigType objects.
 */
export type TemplehsRouteConfigsType = TemplehsRouteConfigType[] | [];
...
/**
 * The TemplehsUtils class provides utility functions for the Templehs project.
 */
class TemplehsUtils {
	/**
	...
	/**
	 * The setRoutes function sets the routes for the Templehs project.
	 */
	static setRoutes(
		config: TemplehsRouteConfigType,
		defaultAuth: TemplehsSettingsConfigType['defaultAuth'] = undefined
	): TemplehsRouteItemType[] {
	...
	static generateRoutesFromConfigs(
		configs: TemplehsRouteConfigsType,
		defaultAuth: TemplehsSettingsConfigType['defaultAuth']
	) {
		let allRoutes: TemplehsRouteItemType[] = [];
		configs.forEach((config: TemplehsRouteConfigType) => {
	...
	static removeNavItem(nav: TemplehsNavigationType, id: string): TemplehsNavigationType {
		return nav
			...
			.filter((s) => s) as TemplehsNavigationType;
	}

	/**
	 * The prependNavItem function prepends a navigation item.
	 */
	static prependNavItem(nav: TemplehsNavigationType, item: TemplehsNavItemType, parentId: string | null): TemplehsNavigationType {
		if (!parentId) {
			return [item, ...nav];
		}
	 ...
	 */
	static appendNavItem(nav: TemplehsNavigationType, item: TemplehsNavItemType, parentId: string | null): TemplehsNavigationType {
		if (!parentId) {
			return [...nav, item];
		}
