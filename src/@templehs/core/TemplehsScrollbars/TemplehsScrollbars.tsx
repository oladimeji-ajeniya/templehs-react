...
import { selectCustomScrollbarsEnabled } from 'src/app/store/templehs/settingsSlice';
...
/**
 * TemplehsScrollbars Component for app-level scrollbar
 * Unlike normal scrollbars, this component supports mobile device as well
 */
const TemplehsScrollbars = forwardRef<HTMLDivElement, TemplehsScrollbarsProps>((props, ref) => {
	const {
		className = '',
		children,
		id = '',
	...
	const hookUpEvents = useCallback(() => {
		Object.keys(handlerNameByEvent).forEach((key) => {
			const callback = props[handlerNameByEvent[key] as keyof TemplehsScrollbarsProps] as (T: HTMLDivElement) => void;
...
export default TemplehsScrollbars;
