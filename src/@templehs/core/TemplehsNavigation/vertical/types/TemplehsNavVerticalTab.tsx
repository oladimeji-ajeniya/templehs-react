...
export type TemplehsNavVerticalTabProps = Omit<TemplehsNavigationProps, 'navigation'> & TemplehsNavItemComponentProps;

/**
 *  The `TemplehsNavVerticalTab` component renders vertical navigation item with an adaptable
 *  layout to be used within the `TemplehsNavigation`. It only supports the `type`s of 'item',
 *  'selection' and 'divider'
 * */
function TemplehsNavVerticalTab(props: TemplehsNavVerticalTabProps) {
			...
			<Root sx={item.sx}>
				<ListItemButton
					...
					className={clsx(
						...
						'templehs-list-item flex flex-col items-center justify-center p-12'
					)}
					...
					{dense ? (
						...
						>
							<div className="relative flex h-32 min-h-32 w-32 items-center justify-center">
								{item.icon ? (
									<TemplehsSvgIcon
										className={clsx('templehs-list-item-icon', item.iconClass)}
										color="action"
									>
										{item.icon}
									</TemplehsSvgIcon>
						...
						<>
							...
							<ListItemText
								className="templehs-list-item-text w-full grow-0"
								primary={item.title}
								classes={{
									primary:
										'text-12 font-medium templehs-list-item-text-primary truncate text-center truncate'
								}}
							/>
						</>
					)}
...
const NavVerticalTab = TemplehsNavVerticalTab;
