import { useDebounce, usePrevious } from 'src/@templehs/hooks';
import { styled } from '@mui/material/styles';
import { Controller, useForm } from 'react-hook-form';
import themeLayoutConfigs, { themeLayoutDefaultsProps } from 'src/app/theme-layouts/themeLayoutConfigs';
...
import { selectUser } from 'src/app/store/user/userSlice';
import { useAppDispatch } from 'src/app/store';
import { Palette } from '@mui/material/styles/createPalette';
import ThemeFormConfigTypes from 'src/@templehs/core/TemplehsSettings/ThemeFormConfigTypes';
...
/**
 * The Root styled component is used to style the root div of the TemplehsSettings component.
 * It uses the styled function from the MUI styles library to create a styled component.
 */
const Root = styled('div')(({ theme }) => ({
	'& .TemplehsSettings-formControl': {
		margin: '6px 0',
		width: '100%',
		'&:last-child': {
			marginBottom: 0
	...
	'& .TemplehsSettings-group': {},

	'& .TemplehsSettings-formGroupTitle': {
		position: 'absolute',
		top: -10,
		left: 8,
		fontWeight: 600,
...
export type TemplehsThemeType = {
	palette: PartialDeep<Palette>;
};

export type TemplehsThemesType = { [key: string]: TemplehsThemeType };

export type TemplehsSettingsConfigType = {
	layout: {
		style?: string;
	...
	direction: 'rtl' | 'ltr';
	theme: {
		main: TemplehsThemeType;
		navbar: TemplehsThemeType;
		toolbar: TemplehsThemeType;
		footer: TemplehsThemeType;
	};
	defaultAuth?: string[];
...
/**
 * The TemplehsSettings component is responsible for rendering the settings form for the Templehs React application.
...
function TemplehsSettings() {
	...
	const form = watch();
	const formConfigs = themeLayoutConfigs[form.layout.style];
	...
	const handleUpdate = useDebounce((newSettings: TemplehsSettingsConfigType) => {
				...
				switch (formControl.type) {
						...
						return (
							<Controller
								...
								render={({ field }) => (
									<FormControl
										component="fieldset"
										className="TemplehsSettings-formControl"
									>
										<FormLabel
											component="legend"
											className="text-14"
										...
										<RadioGroup
											...
											className="TemplehsSettings-group"
					...
					case 'group': {
						return (
							<div
								key={target}
								className="TemplehsSettings-formGroup"
							>
								<Typography
									className="TemplehsSettings-formGroupTitle"
	...
	return (
		<Root>
			<div className="TemplehsSettings-formGroup">
				<Typography
					className="TemplehsSettings-formGroupTitle"
					color="text.secondary"
				>
			...
			<div className="TemplehsSettings-formGroup pb-16">
				<Typography
					className="TemplehsSettings-formGroupTitle"
					color="text.secondary"
				>
...
export default memo(TemplehsSettings);
