...
import { EventType } from '../../app/pages/calendar/types/EventType';
import { LabelType } from '../../app/pages/calendar/types/LabelType';
...
mock.onPost('/api/calendar/labels').reply(({ data }) => {
	const newLabel = { id: TemplehsUtils.generateGUID(), ...JSON.parse(data as string) } as LabelType;
...
mock.onPost('/api/calendar/events').reply(({ data }) => {
	const newEvent = { id: TemplehsUtils.generateGUID(), ...JSON.parse(data as string) } as EventType;
